...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const inviteLink = `https://covivo.lovable.app/invite?token=${token}`;

    const resend = new Resend(resendApiKey);

    const { data, error } = await resend.emails.send({
      from: "Covivo <onboarding@resend.dev>",
      to: [email],
      subject: `${inviterName || "Alguém"} te convidou para ${groupName || "uma moradia compartilhada"} no Covivo`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f4f4f5; margin: 0; padding: 20px;">
          <div style="max-width: 560px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; padding: 40px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05);">
            ...
            <p style="color: #52525b; font-size: 16px; line-height: 1.6; margin: 0 0 24px 0;">
              ${inviterName || "Um morador"} te convidou para participar ${groupName ? `da moradia <strong>${groupName}</strong>` : "de uma moradia compartilhada"} no Covivo.
            </p>
            <p style="color: #52525b; font-size: 16px; line-height: 1.6; margin: 0 0 32px 0;">
              O Covivo ajuda a organizar despesas, pagamentos e a convivência na moradia compartilhada. Clique no botão abaixo para aceitar o convite:
            </p>
