...
BEGIN
    ...
    WITH user_relations AS (
        ...
        SELECT s.user_id as other_user FROM expense_splits s WHERE s.credor_user_id = _user_id AND s.status = 'pending'
        UNION
        SELECT s.credor_user_id as other_user FROM expense_splits s WHERE s.user_id = _user_id AND s.status = 'pending'
    ),
    balances AS (
        SELECT
            ur.other_user,
