-- This migration consolidates the definition of `create_expense_with_splits`.
-- It drops all known previous signatures to ensure a clean state, then creates the final version.

-- Drop the 16-parameter version (if it exists from a partial run)
...
-- Create the final, correct 16-parameter version
