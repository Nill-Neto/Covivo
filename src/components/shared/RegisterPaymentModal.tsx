...
export function RegisterPaymentModal({ open, onOpenChange, expense, cards, participants }: RegisterPaymentModalProps) {
  const { user } = useAuth();
  ...
  const registerPaymentMutation = useMutation({
    mutationFn: async () => {
      if (!expense || !user) throw new Error("Dados insuficientes para registrar pagamento.");
      ...
      const payerName = payerUserId === "me"
        ? "Você"
        : participants.find((p) => p.id === payerUserId)?.name || "Morador";
      const historyLine = `quitada por ${payerName} em ${format(parseLocalDate(paymentDate), "dd/MM")}`;
      const updatedDescription = expense.description
        ? `${expense.description}\n${historyLine}`
        : historyLine;
      
      const actualPayerId = payerUserId === "me" ? user.id : payerUserId;
      const finalCreditCardId = paymentMethod === "credit_card" ? creditCardId : null;
